...
export function PortfolioValueProvider({ children }) {
  ...
  const getSource = useCallback((key) => Number(sources?.[key] || 0), [sources]);

  const total = useMemo(() => Object.values(sources).reduce((a, b) => a + (Number(b) || 0), 0), [sources]);
  ...
  return <PortfolioValueContext.Provider value={value}>{children}</PortfolioValueContext.Provider>;
}
...
export const DEFAULT_DASHBOARD_SOURCES = [PORTFOLIO_SOURCE, HEX_STAKING_SOURCE, EHEX_STAKING_SOURCE];
