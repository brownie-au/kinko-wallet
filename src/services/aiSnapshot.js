...
function normAsset(row = {}) {
  ...
  const address = row.address || row.contract || (String(row.symbol || '').toUpperCase() === 'PLS' ? 'native' : '') || null;
...
function buildSnapshotFromArgs(arg1, arg2) {
  ...
  if (arg1 && typeof arg1 === 'object' && !Array.isArray(arg1) && (arg1.assets || arg1.tokens)) {
    ...
    const totalUsd = Number(arg1.totalUsd) || assets.reduce((a, r) => a + (Number(r.valueUsd) || 0), 0);
    return {
      totalUsd,
      assets,
  ...
  if (Array.isArray(arg1)) {
    ...
    const totalUsd = Number(arg2) || assets.reduce((a, r) => a + (Number(r.valueUsd) || 0), 0);
    return {
      totalUsd,
      assets,
      chains: summariseChains(assets),
