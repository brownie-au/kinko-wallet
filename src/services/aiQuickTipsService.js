...
function buildSummary(portfolio) {
  ...
  const sorted = [...assets].filter((a) => (Number(a.valueUsd) || 0) > 0).sort((a, b) => Number(b.valueUsd) - Number(a.valueUsd));
...
function signatureFrom(summary, riskIndex, timeframe, objective) {
  ...
  const chains = ['eth', 'pulse', 'base', 'other'].map((k) => `${k}:${Math.round(summary.chainPct[k] * 10) / 10}`).join('|');
...
function fallbackTips(summary, riskIndex, timeframe, objective) {
  ...
  if (riskIndex >= 3) {
    tips.push(
      `Risk set to ${['Very Low', 'Low', 'Medium', 'High', 'Very High'][riskIndex]} — predefine drawdown limits and rebalance rules.`
    );
  } else if (riskIndex <= 1) {
    tips.push(
      `Risk set to ${['Very Low', 'Low', 'Medium', 'High', 'Very High'][riskIndex]} — growth may be slower; deploy dry powder on pullbacks.`
    );
  }
  if (tfLabel !== '5y') {
...
export async function getQuickTips({ portfolio, riskIndex = 2, timeframe = '5y', objective = '', force = false }) {
  ...
  try {
    ...
    /* fall through to deterministic tips */
  }
