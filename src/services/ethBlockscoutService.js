...
const BS_HOST = (BS_V2 && BS_V2.replace(/\/api\/v2.*/i, '')) || (BS_V1 && BS_V1.replace(/\/api.*/i, '')) || 'https://eth.blockscout.com';
...
async function fetchV2TokenBalances(address) {
  ...
  for (let guard = 0; guard < 25; guard++) {
    ...
    const items = Array.isArray(json?.items)
      ? json.items
      : Array.isArray(json?.token_balances)
        ? json.token_balances
        : Array.isArray(json)
          ? json
          : [];
    ...
    if (!next) break;
    url = typeof next === 'string' ? (next.startsWith('http') ? next : `${BS_HOST}${next}`) : `${base}?` + new URLSearchParams(next);
  }
  return out;
}
