// src/services/baseBlockscoutService.js
// Base token discovery via Blockscout (no Moralis/Alchemy).
// - Uses env vars: VITE_BASE_BLOCKSCOUT_V2 / VITE_BASE_BLOCKSCOUT_V1
// - Falls back to public base.blockscout.com if env not set
// - Returns: [{ address, symbol, name, decimals, balanceRaw }]

/* ---------- Endpoint setup from .env ---------- */
const BS_V2 = (import.meta.env.VITE_BASE_BLOCKSCOUT_V2 || 'https://base.blockscout.com/api/v2').replace(/\/+$/, '');
const BS_V1 = (import.meta.env.VITE_BASE_BLOCKSCOUT_V1 || 'https://base.blockscout.com/api').replace(/\/+$/, '');
// Host root (e.g., https://base.blockscout.com)
const BS_HOST =
...
/**
 * Public API: get all ERC-20 balances for a Base address from Blockscout.
 */
export async function getBaseTokensFromBlockscout(address, opts = {}) {
  const cacheKey = `kw:bs:base:tokens:${address.toLowerCase()}`;
