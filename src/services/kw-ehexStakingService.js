...
import { cleanHexAddress, checksumUserAddress, isBenignStakeRevert, toNum } from './hexShared';

/* -------------------------------------------------------------------------- */
/* ENV / ADDRS                                                                */
/* -------------------------------------------------------------------------- */

export const HEX_ETH_ADDRESS = (import.meta.env.VITE_ETH_HEX_ADDRESS || '0x2b591e99aFe9F32eaa6214f7B7629768c40eEb39').trim();
...
function getEthRpcUrls() {
  ...
  // CSV list (preferred)
  const csv = import.meta.env.VITE_ETH_RPC_URLS || import.meta.env.VITE_ETHEREUM_RPC_URLS || '';
  if (csv) {
    csv
      .split(',')
      .map((s) => s.trim())
  ...
  urls.push('https://eth.llamarpc.com', 'https://1rpc.io/eth', 'https://cloudflare-eth.com', 'https://rpc.ankr.com/eth');
...
// toNum imported from hexShared

/* Build a pool of Static providers (no auto-detection). */
function buildProviderPool() {
  const urls = getEthRpcUrls();
  return urls.map((url) => new ethers.providers.StaticJsonRpcProvider({ url, timeout: 12000 }, { chainId: 1, name: 'homestead' }));
}
...
async function getStakeCount(addr, providers) {
  try {
    const r = await withProvidersRetry((prov) => contractFor(prov).stakeCount(addr), providers, { retries: Math.max(3, providers.length) });
    return toNum(r);
  } catch (e) {
...
async function getStakeAtIndex(addr, i, providers) {
  const r = await withProvidersRetry((prov) => contractFor(prov).stakeLists(addr, i), providers, {
...
export async function refreshEhexStakesAndCache(arg1, arg2) {
  ...
  if (!successes && fatalNetwork) {
    throw new Error('Network issue (Ethereum). Tried multiple RPCs but none responded. Please check your connection and try again.');
  }
