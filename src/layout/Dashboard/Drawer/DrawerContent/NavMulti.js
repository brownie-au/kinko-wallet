        ...
        {Array.isArray(node.children) &&
          node.children.map((child) =>
            child.type === 'item' ? (
              ...
              <CollapseNode key={child.id} node={child} openMap={openMap} setOpen={setOpen} themeDirection={themeDirection} />
            ) : null
          )}
      </ul>
    </li>
            ...
            {(group.children || []).map((node) =>
              node.type === 'item' ? (
                ...
                <ListGroup key={node.id} className={clsx('pc-item kw-hasmenu border-0 bg-transparent', openMap[node.id] && 'kw-open')}>
                  <CollapseNode node={node} openMap={openMap} setOpen={setOpen} themeDirection={themeDirection} />
                </ListGroup>
              ) : null
            )}
          </ListGroup>
