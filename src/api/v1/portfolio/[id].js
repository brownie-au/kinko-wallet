...
module.exports = async (req, res) => {
  ...
  try {
    if (req.method === 'GET') {
      ...
      return res.status(200).json({ wallets: Array.isArray(obj.wallets) ? obj.wallets : [], updatedAt: obj.updatedAt || 0, checksum: obj.checksum || null });
    }
    ...
    if (req.method === 'PUT') {
      ...
      const put = await fetch(`${URL}/set/${encodeURIComponent(key)}/${payload}`, { method: 'POST', headers: { Authorization: `Bearer ${TOKEN}` } });
