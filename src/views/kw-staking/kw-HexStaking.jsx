...
function readHdsLS() {
  try {
    return JSON.parse(localStorage.getItem(HDS_LS_KEY) || 'null');
  } catch {
    return null;
  }
}
function writeHdsLS(obj) {
  try {
    localStorage.setItem(HDS_LS_KEY, JSON.stringify(obj));
  } catch {}
}
    ...
    for (const key of ['kw:dexscreener:prices:v1', 'kw:dex:prices:v1', 'kw:tokenPrices:v1', 'kw:prices:bySymbol', 'kw:prices:spot:v1']) {
      ...
      const val = Number(
        obj?.HEX ??
          obj?.hex ??
          obj?.['hex:pls'] ??
          (Array.isArray(obj?.tokens) ? Number(obj.tokens.find((t) => (t?.symbol || '').toUpperCase() === 'HEX')?.priceUsd) : undefined)
      );
      if (val > 0) return val;
    }
...
const PLS_HEX_ADDRESS = import.meta.env.VITE_PLS_HEX_ADDRESS || '0x2b591e99aFe9F32eaa6214f7B7629768c40eeb39';
...
function readPriceCache() {
  try {
    return JSON.parse(localStorage.getItem(priceLSKey) || 'null');
  } catch {
    return null;
  }
}
function writePriceCache(obj) {
  try {
    localStorage.setItem(priceLSKey, JSON.stringify(obj));
  } catch {}
}
...
export default function KwHexStaking() {
  ...
  const initialCached = useMemo(() => {
    try {
      return readHexStakesCache(pulseAddresses) || null;
    } catch {
      return null;
    }
  }, [pulseAddresses]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        /* HDS fallback below may kick in */
      }
    })();
  ...
  const sortValue = (r, key) => {
    switch (key) {
      ...
      case 'principalHex':
        return Number(r.principalHex) || 0;
      case 'tShares':
        return Number(r.tShares) || 0;
      case 'lockedDay':
        return Number(r.lockedDay) || 0;
      case 'stakedDays':
        return Number(r.stakedDays) || 0;
      case 'unlockDay':
        return calcUnlockDay(r.lockedDay, r.stakedDays) || 0;
      case 'daysRemaining': {
      ...
      case 'yieldHexTotal':
        return Number(yieldMap[r.id]?.yieldHex || 0);
      case '%apy':
        return Number(yieldMap[r.id]?.apyPct || 0);
      case 'totalHex': {
      ...
      case 'yieldDaily':
        return Number(estimateDailyYieldHex(r.tShares, payoutPerTShareDailyHex) || 0);
      case 'status': {
  ...
  const sortedRows = useMemo(() => {
    ...
    arr.sort((a, b) => {
      ...
      const cmp = typeof av === 'string' || typeof bv === 'string' ? String(av).localeCompare(String(bv)) : (av ?? 0) - (bv ?? 0);
  ...
  const refreshNow = useCallback(async () => {
    ...
    try {
      const payload = await refreshHexStakesAndCache(pulseAddresses, (done, total) => setProgress({ done, total }));
  ...
  const effectivePayoutPerTShare = useMemo(
    () => (Number(payoutPerTShareDailyHex) > 0 ? Number(payoutPerTShareDailyHex) : ppsByDay?.[Number(currentDay) || 0] || 0),
    [payoutPerTShareDailyHex, ppsByDay, currentDay]
  );

  const totalTSharesActive = useMemo(() => (gridRows || []).reduce((s, r) => s + (Number(r.tShares) || 0), 0), [gridRows]);
  ...
  const totalYieldHex = useMemo(() => (gridRows || []).reduce((acc, r) => acc + (Number(r.yieldHex) || 0), 0), [gridRows]);
      ...
      <Row className="gy-3 kw-hex-page">
        ...
        <Col xs={12} className="mt-2">
                      ...
                      return (
                        <tr key={r.id}>
                          <td className="text-start">
                            <span className="kw-wallet-chip">{walletDisplay}</span>
                          </td>
          ...
          {!loading && !filteredRows.length && !filteredRowsEnded.length && !!pulseAddresses.length && !setupError && (
            <Card>
              <Card.Body>No stakes match the current wallet filter.</Card.Body>
            </Card>
          )}
        </Col>
      </Row>
