...
import { Card, Row, Col, Button, Form, Tabs, Tab, Badge, Spinner, Alert } from 'react-bootstrap';
    ...
    <Card className="mb-4">
      <Card.Body>
        ...
        <Row className="align-items-center mb-2">
          <Col>
            <h5 className="mb-1">AI Portfolio Analysis {result && <Badge bg="primary">Generated</Badge>}</h5>
          ...
          <Col xs="auto" className="d-flex align-items-center gap-2 flex-wrap">
            {lastRunAt && (
              <span className="small text-muted">Last run: {lastRunAt.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
            )}
            <Badge bg="info" pill>
              Live
            </Badge>
        ...
        <div className="d-flex flex-wrap align-items-center gap-2 mb-3" style={{ fontSize: 12 }}>
          {chainChips.chips.length > 0 ? (
            chainChips.chips.map((c) => (
              ...
              >
                <span
                  style={{ width: 10, height: 10, borderRadius: '50%', display: 'inline-block', marginRight: 8, backgroundColor: c.color }}
                />
        ...
        <Row className="g-3">
          ...
          <Col xs={12} md={4}>
            ...
            <RiskSlider value={riskIndex} onChange={setRiskIndex} disabled={loading} />

            <Form.Label className="mt-3 mb-1">Timeframe</Form.Label>
            <Form.Select value={timeframe} onChange={(e) => setTimeframe(e.target.value)} disabled={loading}>
              {TIMEFRAMES.map((t) => (
            ...
            <div className="d-flex flex-wrap gap-2 mt-3">
              ...
              <Button
                ...
                disabled={loading || !result}
                title="Download as TXT or print to PDF"
              >
                Download
              </Button>
            </div>
            ...
            <Tabs defaultActiveKey="overview" className="mb-0">
              ...
              <Tab eventKey="news" title="AI News Brief">
                <ScrollablePane>
                  ...
                  {!loading && result?.news ? <PreJson data={result.news} /> : <MutedNote>No news items in this response.</MutedNote>}
                </ScrollablePane>
              </Tab>
            </Tabs>
          </Col>
...
/* RiskSlider — 5-step slider with marks and live label */
function RiskSlider({ value = 2, onChange, disabled }) {
  const marks = RISK_LEVELS;
  const pct = Math.max(0, Math.min(100, (Number(value) / 4) * 100));

  return (
    <div>
      <div className="position-relative" style={{ paddingTop: 18 }}>
        {/* Bubble label */}
        <div
          style={{
            position: 'absolute',
        ...
        <input
          ...
          style={{ width: '100%' }}
          className="kw-risk-range"
        />
        {/* Marks */}
        <div className="d-flex justify-content-between mt-1" style={{ fontSize: 11 }}>
          <span className="text-muted">Very Low</span>
          <span className="text-muted">Low</span>
          <span className="text-muted">Medium</span>
          <span className="text-muted">High</span>
          <span className="text-muted">Very High</span>
        </div>
      </div>

      {/* Inline styles for the slider */}
      <style>{`
        .kw-risk-range { appearance: none; height: 6px; border-radius: 6px; background: linear-gradient(90deg,#10b981, #f59e0b 50%, #ef4444); outline: none; }
        .kw-risk-range::-webkit-slider-thumb { appearance: none; width: 18px; height: 18px; border-radius: 50%; background: #fff; border: 2px solid #333; box-shadow: 0 0 0 2px rgba(0,0,0,0.15); cursor: pointer; }
        .kw-risk-range::-moz-range-thumb { width: 18px; height: 18px; border-radius: 50%; background: #fff; border: 2px solid #333; box-shadow: 0 0 0 2px rgba(0,0,0,0.15); cursor: pointer; }
      `}</style>
    </div>
  );
}
...
function buildReportText(result) {
  if (!result) return '';
  const parts = [];
  if (result.overviewHtml) parts.push('=== Overview ===\n' + htmlToText(result.overviewHtml));
  if (result.observationsHtml) parts.push('=== Observations ===\n' + htmlToText(result.observationsHtml));
  if (result.breakdownHtml) parts.push('=== Breakdown ===\n' + htmlToText(result.breakdownHtml));
  if (result.scorecard) {
    const total = Math.round(Number(result.scorecard.total || 0));
    parts.push('=== Scorecard ===\n' + `Composite: ${total}/100`);
  }
  if (result.news) parts.push('=== News ===\n' + JSON.stringify(result.news, null, 2));
  return parts.filter(Boolean).join('\n\n');
}
async function copyReport(result) {
  const txt = buildReportText(result);
  if (!txt) return;
  try {
...
function downloadReport(result) {
  ...
  a.href = url; a.download = 'kinko-analysis.txt'; a.click();
  setTimeout(() => URL.revokeObjectURL(url), 2000);

  // Also open print dialog for PDF (user can choose Save as PDF)
  const w = window.open('', '_blank');
  if (w) {
    const safeHtml = (result.overviewHtml || '') + (result.observationsHtml || '') + (result.breakdownHtml || '');
    w.document.write(`<html><head><title>Kinko Analysis</title><style>body{font-family:system-ui,Segoe UI,Roboto,Arial,sans-serif;padding:20px;line-height:1.5}</style></head><body>${safeHtml}</body></html>`);
    w.document.close();
    w.focus();
    w.print();
  }
