// src/components/kw-EHexStakingHeaderContainer.jsx
import React, { useMemo, useEffect, useState } from 'react';
import KwEHexStakingHeader from './kw-EHexStakingHeader.jsx';
...
/** Read any cached EHEX price (DexScreener-first; no Moralis). */
function readEhexPriceUsdFromCaches(override) {
  if (typeof override === 'number' && override > 0) return override;
  try {
    const k1 = localStorage.getItem('kw:lastEHEXPriceUsd');
    if (k1) {
      const v = Number(k1);
      if (v > 0) return v;
    }
    ...
    for (const key of [
      'kw:dexscreener:prices:v1', // { EHEX: 0.01 }
      'kw:dex:prices:v1',
      'kw:topTokensCache' // { tokens:[{symbol:'EHEX', priceUsd:...}] }
    ]) {
      const raw = localStorage.getItem(key);
      if (!raw) continue;
      const obj = JSON.parse(raw);
      if (key === 'kw:topTokensCache' && Array.isArray(obj?.tokens)) {
        const token = obj.tokens.find((t) => (t?.symbol || '').toUpperCase() === 'EHEX');
        const v = Number(token?.priceUsd);
        if (v > 0) return v;
      } else {
        const v = Number(obj?.EHEX ?? obj?.ehex ?? obj?.['hex:eth'] ?? obj?.['ehex:eth']);
        if (v > 0) return v;
      }
    }
    ...
    for (const key of ['kw:tokenPrices:v1', 'kw:prices:bySymbol', 'kw:prices:spot:v1']) {
      ...
      const v = Number(map?.EHEX ?? map?.ehex ?? map?.['hex:eth'] ?? map?.['ehex:eth']);
      if (v > 0) return v;
    }

    if (typeof window !== 'undefined') {
      const v = Number(window.__KW_EHEX_PRICE_USD);
      if (v > 0) return v;
    }
  } catch {}
  return 0;
...
function readTotalYieldFromDom(unitUpper = 'EHEX') {
  try {
    const tables = Array.from(document.querySelectorAll('table'));
    for (const table of tables) {
      const idx = findHeaderIndex(table, (h) => {
        const t = h.trim().toUpperCase();
        return t === 'YIELD' || t === `YIELD (${unitUpper})` || t.startsWith('YIELD');
      });
      if (idx === -1) continue;
      const tbody = table.tBodies?.[0] || table.querySelector('tbody');
      if (!tbody) continue;
...
function readTotalHexFromDom(unitUpper = 'EHEX') {
  try {
...
function readEhexPriceUsdFromDomTicker() {
  try {
    ...
    for (const scope of scopes) {
      ...
      const m = txt.match(/EHEX\s+USD\s*\$?\s*([0-9]*\.?[0-9]+)/i);
      if (m && m[1]) {
        const n = Number(m[1]);
...
/* ---------------- Component ---------------- */
export default function KwEHexStakingHeaderContainer({
  stakes,
  gridRows,
  currentHexDay,
  ...
  const [priceDom, setPriceDom] = useState(null);
  ...
  useEffect(() => {
    const id = setTimeout(() => {
      ...
      const yld = readTotalYieldFromDom(unitUpper);
      ...
      const px = typeof hexPriceUsdOverride === 'number' && hexPriceUsdOverride > 0 ? hexPriceUsdOverride : readEhexPriceUsdFromDomTicker();
      if (px !== null && px !== undefined) setPriceDom(px);
    }, 150);
  ...
  const priceFromCache = useMemo(() => readEhexPriceUsdFromCaches(hexPriceUsdOverride), [hexPriceUsdOverride]);
  const finalPriceUsd = priceDom && priceDom > 0 ? priceDom : priceFromCache;

  const totalUsd = finalPriceUsd > 0 ? finalTotalHex * finalPriceUsd : 0;
